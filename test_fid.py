...
inference_folder = str(os.getcwd())+"/results-CycleT1"
os.makedirs(inference_folder, exist_ok=True)

original_folder = str(os.getcwd())+"/original-Cycle"
...
def test(args):
    ...
    a_test_loader = torch.utils.data.DataLoader(a_test_data, batch_size=args.batch_size, shuffle=False)
    b_test_loader = torch.utils.data.DataLoader(b_test_data, batch_size=args.batch_size, shuffle=False)
 ...
 # for b test dataset - corresponds to T1 images
    for imagesT1, imagesT2 in zip(b_real_test, a_real_test):    
          ...
          with torch.no_grad():
            ...
            bfake_to_cpu = bfake_to_cpu.view(1, 3, 256, 256)
            ...
            translated=(bfake_to_cpu + 1)/ 2.0
            original=(imagesT1 + 1)/ 2.0
            
            torchvision.utils.save_image(original, '/home/hs21/cycleGAN-PyTorch/original-Cycle/image' + str(index) + '.png')
            torchvision.utils.save_image(translated, '/home/hs21/cycleGAN-PyTorch/results-CycleT1/image' + str(index) + '.png') 
